...
BSTNode* insert_helper(BSTNode* root, BSTNode* parent, int value){
    if(root == nullptr){
        root = new BSTNode(value, parent);
    }
    else{
        if(value < root->data){
            root->left = insert_helper(root->left, root, value);
        }
        else{
            root->right = insert_helper(root->right, root, value);
        }
    }
...
BSTNode* insert(BSTNode* root, int value){
    return insert_helper(root, nullptr, value);
}
